/*****************************************************************************************
   ...
   2025-01-19 JMS                  Ligths commented. Pit added using same lap detectors.
...
const byte laneToInterrupMapping[] = {  2,  3, 18, 19, 20, 21 };
const byte laneToRelayMapping[]    = { 29, 30, 31, 32, 33, 34 };
...
class Lane {
  protected:
    ...
    volatile unsigned long finish = 0;
  ...
  public:
    ...
    void checkLapOrPit()
    {
      ...
      { // si se inicio una detección
        ...
        laneDetect = digitalRead(laneToInterrupMapping[lane]);
...
void allRelaysOn() {
  for(byte i = 0; i<20; i++)
  { /* several times to avoid physical problemas */
  relay1.on();
  relay2.on();
  relay3.on();
  relay4.on();
...
void attachAllInterrupts() {
  attachInterrupt(digitalPinToInterrupt(LANE_1), laneDetect1, ISRDETECTIONSIDE);
  attachInterrupt(digitalPinToInterrupt(LANE_2), laneDetect2, ISRDETECTIONSIDE);
  attachInterrupt(digitalPinToInterrupt(LANE_3), laneDetect3, ISRDETECTIONSIDE);
  attachInterrupt(digitalPinToInterrupt(LANE_4), laneDetect4, ISRDETECTIONSIDE);
  attachInterrupt(digitalPinToInterrupt(LANE_5), laneDetect5, ISRDETECTIONSIDE);
  attachInterrupt(digitalPinToInterrupt(LANE_6), laneDetect6, ISRDETECTIONSIDE);
}
